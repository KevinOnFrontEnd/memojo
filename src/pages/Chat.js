...
const Chat = () => {
  ...
  useEffect(() => {
    // Optional: Fetch messages for this specific address
    // Assuming transactionDetails are globally updated for the app.
    console.log('Chat loaded for address:', address);
    const chatMessages = transactionDetails[address];
  ...
  return (
    ...
    {chatMessages.map((item) => {
      if (item.type === 1) { //sent transactions with memos
          return (
            <div className="w-100 pb-2 pb-4 flex justify-end" key={item.confirmed_height}>
              <div className="min-w-[50%] max-w-[50%] flex flex-col">
                <div className={item.confirmed_height == 0 ? "bg-pink-200 text-white p-3 rounded-2xl" : "bg-blue-500 text-white p-3 rounded-2xl"}>
                  <span className="block text-gray-800">
                    {item.message}
                  </span>
                </div>
                <div className="text-xs text-gray-400 mt-2 self-end text-right">
                  <span>Block Height: {item.confirmed_height == 0 ? "Pending" : item.confirmed_height}</span>
                </div>
              </div>
            </div>
          );
      }
      else if(item.type === 0) { //received transactions with memos
          return (
      ...
      <div class="p-4 bg-white border-t border-gray-300">
        <div class="flex items-center space-x-3">
          ...
          />
          <button disabled={sendButtonDisabled} onClick={handleSendMessage} class={sendButtonDisabled == false ? "bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600" : "bg-gray-500 text-white px-4 py-2 rounded-lg"}>
            Send
          </button>
        </div>
      </div>
